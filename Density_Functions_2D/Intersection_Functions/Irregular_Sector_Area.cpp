...
{
    if (abs( Norm( vertex, c.c ) - c.r ) < tiny_num) // Vertex on boundary.
    {
        ...
        else
        {
            ...
            else
            {
                ...
                if (e1.line.oriented_side( vertex + v2 ) == ON_POSITIVE_SIDE) // Deciding orientation of the two lines.
                {
    ...
    else if (Norm( vertex, c.c ) > c.r) // Vertex outside of circle.
    {
        double vol1 = 0, vol2 = 0;
